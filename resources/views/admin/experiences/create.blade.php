...
@section('page_heading', 'Tambah Pengalaman')
    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
            ...
            <div class="p-6 bg-rose-950 text-white border-b border-rose-900">
                <h2 class="text-lg font-bold">Form Tambah Riwayat Pengalaman</h2>
                <p class="text-xs text-rose-200/80 mt-1">Lengkapi informasi pengalaman kerja atau organisasi Anda di bawah ini.</p>
            </div>

            <!-- BODY FORM -->
            <form action="{{ route('admin.experiences.store') }}" method="POST" enctype="multipart/form-data"
                class="p-6 space-y-6" x-data="{ isCurrent: {{ old('is_current') ? 'true' : 'false' }} }">
                @csrf
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4"
                        ...
                        class="w-full px-4 py-2.5 rounded-xl border {{ $errors->has('description') ? 'border-red-500' : 'border-slate-300' }} focus:border-rose-800 focus:ring-2 focus:ring-rose-800/20 text-slate-800 placeholder-slate-400 text-sm transition outline-none">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1.5 font-medium">{{ $message }}</p>
                    @enderror
                </div>
