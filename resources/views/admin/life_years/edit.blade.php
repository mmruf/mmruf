...
@section('title', 'Edit Life Years')
@section('page_heading', 'Edit Life Years')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
        ...
        <div class="p-6 bg-rose-950 text-white border-b border-rose-900">
            <h2 class="text-lg font-bold">Form Edit Perjalanan Tahun (Life Year)</h2>
            <p class="text-xs text-rose-200/80 mt-1">Perbarui catatan penting atau momen bersejarah pada tahun ini.</p>
        </div>

        <!-- BODY FORM -->
        <form action="{{ route('admin.life-years.update', $lifeYear->id) }}" 
              method="POST" 
              class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" 
                       name="title" 
                       id="title" 
                       value="{{ old('title', $lifeYear->title) }}"
            ...
            <div>
                ...
                <textarea name="summary" 
                          ...
                          class="w-full px-4 py-2.5 rounded-xl border {{ $errors->has('summary') ? 'border-red-500' : 'border-slate-300' }} focus:border-rose-800 focus:ring-2 focus:ring-rose-800/20 text-slate-800 placeholder-slate-400 text-sm transition outline-none">{{ old('summary', $lifeYear->summary) }}</textarea>
                @error('summary')
                    <p class="text-red-500 text-xs mt-1.5 font-medium">{{ $message }}</p>
                @enderror
            </div>
